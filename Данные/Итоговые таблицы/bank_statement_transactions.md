...
После объединения файлов пайплайн удаляет бизнес-дубли операций. Дублем считается строка,
у которой совпадают все поля банковской операции, кроме технических полей источника
(`source_file`, `source_file_path`, `source_file_hash`, `source_sheet`, `source_row_number`).
Это защищает итоговую таблицу от повторной загрузки операций из пересекающихся по датам выписок.
